...
static inline void
spa_pod_builder_get_state(struct spa_pod_builder *builder,
                          struct spa_pod_builder_state *state) {
  *state = builder->state;
}
...
static inline void spa_pod_builder_reset(struct spa_pod_builder *builder,
                                         struct spa_pod_builder_state *state) {
...
static inline void spa_pod_builder_init(struct spa_pod_builder *builder,
...
static inline struct spa_pod *
spa_pod_builder_frame(struct spa_pod_builder *builder,
                      struct spa_pod_frame *frame) {
...
static inline void spa_pod_builder_push(struct spa_pod_builder *builder,
                                        struct spa_pod_frame *frame,
  ...
  if (frame->pod.type == SPA_TYPE_Array || frame->pod.type == SPA_TYPE_Choice)
    builder->state.flags =
        SPA_POD_BUILDER_FLAG_FIRST | SPA_POD_BUILDER_FLAG_BODY;
}

static inline int spa_pod_builder_raw(struct spa_pod_builder *builder,
  ...
  if (offset + size > builder->size) {
    ...
    if (offset <= builder->size)
      spa_callbacks_call_res(&builder->callbacks,
                             struct spa_pod_builder_callbacks, res, overflow, 0,
                             offset + size);
  }
  if (res == 0 && data)
...
static inline int spa_pod_builder_pad(struct spa_pod_builder *builder,
                                      uint32_t size) {
  uint64_t zeroes = 0;
...
static inline int spa_pod_builder_raw_padded(struct spa_pod_builder *builder,
                                             const void *data, uint32_t size) {
...
static inline int spa_pod_builder_primitive(struct spa_pod_builder *builder,
...
static inline int spa_pod_builder_child(struct spa_pod_builder *builder,
...
#define SPA_POD_INIT_Bool(val)                                                 \
  ((struct spa_pod_bool){{sizeof(uint32_t), SPA_TYPE_Bool}, (val) ? 1 : 0, 0})

static inline int spa_pod_builder_bool(struct spa_pod_builder *builder,
                                       bool val) {
...
#define SPA_POD_INIT_Id(val)                                                   \
  ((struct spa_pod_id){{sizeof(uint32_t), SPA_TYPE_Id}, (val), 0})

static inline int spa_pod_builder_id(struct spa_pod_builder *builder,
                                     uint32_t val) {
...
#define SPA_POD_INIT_Int(val)                                                  \
  ((struct spa_pod_int){{sizeof(int32_t), SPA_TYPE_Int}, (val), 0})

static inline int spa_pod_builder_int(struct spa_pod_builder *builder,
                                      int32_t val) {
...
#define SPA_POD_INIT_Long(val)                                                 \
  ((struct spa_pod_long){{sizeof(int64_t), SPA_TYPE_Long}, (val)})

static inline int spa_pod_builder_long(struct spa_pod_builder *builder,
                                       int64_t val) {
...
#define SPA_POD_INIT_Float(val)                                                \
  ((struct spa_pod_float){{sizeof(float), SPA_TYPE_Float}, (val), 0})

static inline int spa_pod_builder_float(struct spa_pod_builder *builder,
                                        float val) {
...
#define SPA_POD_INIT_Double(val)                                               \
  ((struct spa_pod_double){{sizeof(double), SPA_TYPE_Double}, (val)})

static inline int spa_pod_builder_double(struct spa_pod_builder *builder,
                                         double val) {
...
#define SPA_POD_INIT_String(len)                                               \
  ((struct spa_pod_string){{(len), SPA_TYPE_String}})

static inline int spa_pod_builder_write_string(struct spa_pod_builder *builder,
...
static inline int spa_pod_builder_string_len(struct spa_pod_builder *builder,
                                             const char *str, uint32_t len) {
...
static inline int spa_pod_builder_string(struct spa_pod_builder *builder,
                                         const char *str) {
...
#define SPA_POD_INIT_Bytes(len)                                                \
  ((struct spa_pod_bytes){{(len), SPA_TYPE_Bytes}})

static inline int spa_pod_builder_bytes(struct spa_pod_builder *builder,
...
#define SPA_POD_INIT_Pointer(type, value)                                      \
  ((struct spa_pod_pointer){                                                   \
      {sizeof(struct spa_pod_pointer_body), SPA_TYPE_Pointer},                 \
      {(type), 0, (value)}})

static inline int spa_pod_builder_pointer(struct spa_pod_builder *builder,
                                          uint32_t type, const void *val) {
  const struct spa_pod_pointer p = SPA_POD_INIT_Pointer(type, val);
  return spa_pod_builder_primitive(builder, &p.pod);
}

#define SPA_POD_INIT_Fd(fd)                                                    \
  ((struct spa_pod_fd){{sizeof(int64_t), SPA_TYPE_Fd}, (fd)})

static inline int spa_pod_builder_fd(struct spa_pod_builder *builder,
                                     int64_t fd) {
...
#define SPA_POD_INIT_Rectangle(val)                                            \
  ((struct spa_pod_rectangle){                                                 \
      {sizeof(struct spa_rectangle), SPA_TYPE_Rectangle}, (val)})

static inline int spa_pod_builder_rectangle(struct spa_pod_builder *builder,
                                            uint32_t width, uint32_t height) {
  const struct spa_pod_rectangle p =
      SPA_POD_INIT_Rectangle(SPA_RECTANGLE(width, height));
  return spa_pod_builder_primitive(builder, &p.pod);
}

#define SPA_POD_INIT_Fraction(val)                                             \
  ((struct spa_pod_fraction){{sizeof(struct spa_fraction), SPA_TYPE_Fraction}, \
                             (val)})

static inline int spa_pod_builder_fraction(struct spa_pod_builder *builder,
                                           uint32_t num, uint32_t denom) {
  const struct spa_pod_fraction p =
      SPA_POD_INIT_Fraction(SPA_FRACTION(num, denom));
  return spa_pod_builder_primitive(builder, &p.pod);
}

static inline int spa_pod_builder_push_array(struct spa_pod_builder *builder,
                                             struct spa_pod_frame *frame) {
  const struct spa_pod_array p = {
      {sizeof(struct spa_pod_array_body) - sizeof(struct spa_pod),
       SPA_TYPE_Array},
      {{0, 0}}};
  uint32_t offset = builder->state.offset;
  int res =
...
static inline int spa_pod_builder_array(struct spa_pod_builder *builder,
                                        uint32_t child_size,
                                        uint32_t child_type, uint32_t n_elems,
                                        const void *elems) {
  const struct spa_pod_array p = {
      {(uint32_t)(sizeof(struct spa_pod_array_body) + n_elems * child_size),
       SPA_TYPE_Array},
      {{child_size, child_type}}};
  int r, res = spa_pod_builder_raw(builder, &p, sizeof(p));
  if ((r = spa_pod_builder_raw_padded(builder, elems, child_size * n_elems)) <
      0)
    res = r;
  return res;
}
...
#define SPA_POD_INIT_Choice(type, ctype, child_type, n_vals, ...)              \
  ((struct {                                                                   \
    ...
    ctype vals[(n_vals)];                                                      \
  }){{{(n_vals) * sizeof(ctype) + sizeof(struct spa_pod_choice_body),          \
       SPA_TYPE_Choice},                                                       \
      {(type), 0, {sizeof(ctype), (child_type)}}},                             \
     {__VA_ARGS__}})

static inline int spa_pod_builder_push_choice(struct spa_pod_builder *builder,
                                              struct spa_pod_frame *frame,
                                              uint32_t type, uint32_t flags) {
  const struct spa_pod_choice p = {
      {sizeof(struct spa_pod_choice_body) - sizeof(struct spa_pod),
       SPA_TYPE_Choice},
      {type, flags, {0, 0}}};
  uint32_t offset = builder->state.offset;
  int res =
...
#define SPA_POD_INIT_Struct(size)                                              \
  ((struct spa_pod_struct){{(size), SPA_TYPE_Struct}})

static inline int spa_pod_builder_push_struct(struct spa_pod_builder *builder,
                                              struct spa_pod_frame *frame) {
...
static inline int spa_pod_builder_push_object(struct spa_pod_builder *builder,
                                              struct spa_pod_frame *frame,
...
static inline int spa_pod_builder_prop(struct spa_pod_builder *builder,
...
static inline int spa_pod_builder_push_sequence(struct spa_pod_builder *builder,
                                                struct spa_pod_frame *frame,
                                                uint32_t unit) {
...
static inline uint32_t spa_pod_builder_control(struct spa_pod_builder *builder,
...
#define SPA_POD_BUILDER_COLLECT(builder, type, args)                           \
  do {                                                                         \
    ...
    case 'R': {                                                                \
      struct spa_rectangle *rectval = va_arg(args, struct spa_rectangle *);    \
      spa_pod_builder_rectangle(builder, rectval->width, rectval->height);     \
      break;                                                                   \
    }                                                                          \
    case 'F': {                                                                \
      struct spa_fraction *fracval = va_arg(args, struct spa_fraction *);      \
    ...
    case 'a': {                                                                \
      ...
      spa_pod_builder_array(builder, child_size, child_type, n_elems, elems);  \
      break;                                                                   \
    }                                                                          \
    case 'p': {                                                                \
...
static inline int spa_pod_builder_addv(struct spa_pod_builder *builder,
                                       va_list args) {
  int res = 0;
...
static inline struct spa_pod *spa_pod_copy(const struct spa_pod *pod) {
  size_t size;
  struct spa_pod *c;
